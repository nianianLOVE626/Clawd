...
object ClawdMcpServer {
    ...
    private const val SERVER_VERSION = "2.1.0"
    ...
    fun start(context: Context): Result<Int> = runCatching {
        ...
        val host = AppState.mcpBindHost.trim().ifBlank { "127.0.0.1" }
        val bindAddress = InetAddress.getByName(host)
        val ss = ServerSocket(port, 64, bindAddress)
        server = ss
        running = true
        pool.execute {
            while (running) {
                try {
                    pool.execute { handle(ss.accept()) }
    ...
    /** Real IPv4 addresses currently exposed by Android network interfaces. Never returns a fake placeholder. */
    ...
    fun endpoint(): String = if (AppState.mcpBindHost == "0.0.0.0") {
        lanAddresses().firstOrNull()?.let { "http://$it:${AppState.mcpServerPort}/mcp" } ?: loopbackEndpoint()
    } else loopbackEndpoint()

    fun loopbackEndpoint() = "http://127.0.0.1:${AppState.mcpServerPort}/mcp"

    fun lanEndpoint(): String? = lanAddresses().firstOrNull()?.let {
        "http://$it:${AppState.mcpServerPort}/mcp"
    }

    fun sseEndpoint(): String = if (AppState.mcpBindHost == "0.0.0.0") {
        lanAddresses().firstOrNull()?.let { "http://$it:${AppState.mcpServerPort}/sse" } ?: ""
    } else "http://127.0.0.1:${AppState.mcpServerPort}/sse"

    /** Performs an actual HTTP MCP initialize + tools/list request against the running local server. */
    fun selfTest(): Diagnostic {
        val addresses = lanAddresses()
        if (!running) return Diagnostic(false, AppState.mcpBindHost, AppState.mcpServerPort, addresses, false, false, "MCP 尚未启动")
        val loop = testEndpoint(loopbackEndpoint())
        val lan = lanEndpoint()?.let { testEndpoint(it) } ?: false
        val detail = when {
            loop && lan -> "本机与局域网地址均可完成 initialize + tools/list"
            loop -> if (AppState.mcpBindHost == "0.0.0.0") "本机 MCP 正常；局域网地址未通过自测" else "本机 MCP 正常"
            else -> "MCP 已监听，但 initialize/tools/list 自测失败"
        }
        return Diagnostic(running, AppState.mcpBindHost, AppState.mcpServerPort, addresses, loop, lan, detail)
    }

    private fun testEndpoint(endpoint: String): Boolean = runCatching {
        val initialize = JSONObject()
            .put("jsonrpc", "2.0").put("id", 1).put("method", "initialize")
            .put("params", JSONObject()
                .put("protocolVersion", PROTOCOL)
                .put("capabilities", JSONObject())
                .put("clientInfo", JSONObject().put("name", "ClawdSelfTest").put("version", "1.0")))
        val init = postJson(endpoint, initialize.toString())
        val initObj = JSONObject(init)
        if (initObj.has("error")) return false
        val list = JSONObject().put("jsonrpc", "2.0").put("id", 2).put("method", "tools/list").put("params", JSONObject())
        val listed = JSONObject(postJson(endpoint, list.toString()))
        val tools = listed.optJSONObject("result")?.optJSONArray("tools") ?: return false
        tools.length() > 0
    }.getOrDefault(false)
    ...
    private fun postJson(endpoint: String, body: String): String {
        val c = (URL(endpoint).openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("MCP-Protocol-Version", PROTOCOL)
            setRequestProperty("Mcp-Method", JSONObject(body).optString("method"))
            setRequestProperty("Mcp-Name", SERVER_NAME)
            AppState.mcpServerToken.trim().takeIf { it.isNotBlank() }?.let { setRequestProperty("Authorization", "Bearer $it") }
        }
    ...
    private fun handle(s: Socket) {
        s.use { socket ->
            ...
            when {
                method == "GET" && path == "/health" -> respond(socket, 200, "application/json", JSONObject().put("ok", true).put("running", running).put("name", SERVER_NAME).put("port", AppState.mcpServerPort).put("addresses", JSONArray(lanAddresses())).toString())
                method == "GET" && path == "/mcp" -> respond(socket, 200, "application/json", JSONObject().put("transport", "streamable-http").put("endpoint", endpoint()).put("loopbackEndpoint", loopbackEndpoint()).put("lanEndpoints", JSONArray(lanAddresses().map { "http://$it:${AppState.mcpServerPort}/mcp" })).put("protocolVersion", PROTOCOL).toString())
    ...
    private fun authorized(h: Map<String, String>): Boolean {
        val expected = AppState.mcpServerToken.trim()
        if (expected.isBlank()) return true
        val supplied = h["authorization"]?.removePrefix("Bearer ")?.trim()
        return supplied == expected
    }

    private fun handleRpc(s: Socket, body: String, headers: Map<String, String>) {
        val req = runCatching { JSONObject(body) }.getOrElse {
            respond(s, 400, "application/json", errorResponse(JSONObject.NULL, -32700, "Parse error").toString()); return
        }
    ...
    private fun handleSse(s: Socket) {
        val session = UUID.randomUUID().toString()
        val host = if (AppState.mcpBindHost == "0.0.0.0") lanAddresses().firstOrNull() ?: "127.0.0.1" else "127.0.0.1"
        val endpointUrl = "http://$host:${AppState.mcpServerPort}/message?sessionId=$session"
        val payload = "event: endpoint\ndata: $endpointUrl\n\n"
        val out = s.getOutputStream()
        out.write(("HTTP/1.1 200 OK\r\nContent-Type: text/event-stream; charset=utf-8\r\nCache-Control: no-cache\r\nConnection: keep-alive\r\nAccess-Control-Allow-Origin: *\r\n\r\n$payload").toByteArray(StandardCharsets.UTF_8))
        out.flush()
        runCatching { Thread.sleep(30000) }
    }
    ...
    private fun tools() = JSONArray().apply {
        ...
        put(tool("clawd_open_chat", "打开 Clawd 的独立聊天窗口", JSONObject().put("type", "object").put("properties", JSONObject())))
        put(tool("clawd_get_last_user_message", "读取 Clawd 聊天框里最近一次用户输入，供 Operit AI 处理", JSONObject().put("type", "object").put("properties", JSONObject())))
    }

    private fun tool(n: String, d: String, s: JSONObject) = JSONObject().put("name", n).put("description", d).put("inputSchema", s)

    private fun schema(vararg p: String): JSONObject {
        val props = JSONObject(); val req = JSONArray(); var i = 0
        while (i + 1 < p.size) { props.put(p[i], JSONObject().put("type", p[i + 1])); req.put(p[i]); i += 2 }
        return JSONObject().put("type", "object").put("properties", props).put("required", req)
    }

    private fun callTool(p: JSONObject): JSONObject {
        val n = p.optString("name"); val a = p.optJSONObject("arguments") ?: JSONObject()
        return when (n) {
            ...
            "clawd_open_chat" -> ok(if(ClawdPetController.openChat()) "聊天框已打开" else "Clawd 未运行")
            "clawd_get_last_user_message" -> ok(ClawdPetController.lastMessage.ifBlank { "暂无用户消息" })
            "clawd_voice" -> {
                val text = a.optString("text")
                val f = VoiceApi.synthesize(AppState.context(), text).getOrNull()
                if (f != null) { ClawdPetController.showBubble("正在说：${text.take(40)}", 8); VoicePlayer.play(f); ok("语音已播放") } else ok("语音生成失败")
            }
    ...
    private fun respond(s: Socket, code: Int, type: String, body: String, sessionId: String? = null) {
        ...
        out.write(("HTTP/1.1 $code $reason\r\nContent-Type: $type; charset=utf-8\r\nContent-Length: ${b.size}\r\nConnection: close\r\nAccess-Control-Allow-Origin: *\r\nAccess-Control-Allow-Headers: Content-Type, Authorization, Accept, Mcp-Session-Id, MCP-Protocol-Version, Mcp-Method, Mcp-Name\r\nAccess-Control-Expose-Headers: Mcp-Session-Id\r\nAccess-Control-Allow-Methods: GET, POST, OPTIONS\r\n$extra\r\n").toByteArray(StandardCharsets.UTF_8))
